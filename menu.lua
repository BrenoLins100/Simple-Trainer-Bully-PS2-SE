-- PT-BR
-- SIMPLE TRAINER V1 BY: BRENO LINS
-- NAO ROUBE OU FACA REUPLOAD DESTE MOD!
-- AUTOR: BRENO LINS https://github.com/brenolins100

-- EN

-- SIMPLE TRAINER V1 BY: BRENO LINS
-- DO NOT STEAL OR REUPLOAD THIS SOURCE!
-- AUTHOR: BRENO LINS https://github.com/brenolins100
  ...
  repeat
   ...
   -- TextPrintString("Navigation: ~dleft~ ~dright~ \n  Confirm: ~ddown~ \n  Back: ~LOOK_BACK~  ",4,2)
    ...
    --if IsButtonBeingPressed(7,0) then
     ...
     -- ShowMenuPos()
    --  Wait(0)
     -- PlayerSwapModel("TO_HoboSanta")
     -- PedSetActionTree(gPlayer,"/Global/BOSS_Russell","Act/Anim/BOSS_Russell.act") -- estilo russel
...
function TeleTransporte()
  ...
  repeat
    ...
    for j = loopStart,loopEnd do 
      areasString = areasString .. (j == s and "->" or "")..options[j].areaName .. "\n"
  ...
  function OutfitSelector()
    local options = {
      ...
      {model = "Columbus",name ="Columbus"},
      {model = "Nutcracker",name ="Nutrackin"},
      {model = "Elf",name ="Elf"},
      {model = "Marching Band",name ="Marching Band"},
      {model = "Nascar",name ="Nascar"},
      {model = "80 Rocker",name ="Rocker"},
      {model = "Panda",name ="Panda"},
      {model = "Alien",name ="Alien"},
      {model = "NerdJimmy", name ="Nerd"}
    }
    local s = 1
    repeat
      ...
      for j = loopStart, loopEnd do 
        clothesString = clothesString .. (j == s and "->" or "")..options[j].name .. "\n" 
      end
      TextPrintString(clothesString,1,1)
...
function SetPedWeapon()
  ...
  repeat
    ...
    local weaponsPedString = ""
      ...
      for j = loopStart, loopEnd do
        weaponsPedString = weaponsPedString .. (s == j and "--" or "")..options[j].name .. (s == j and "--" or "") .. "\n"
      end

      TextPrintString(weaponsPedString,1,1)
